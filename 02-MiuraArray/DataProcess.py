...
class DataPostProcess(object):
    ...
    def Process(self):
        ...
        InitialEnergy = 4 * self.stiffness * self.dx / 2.0 * 19 * (pi/2) ** 2
        ...
        x1 = DirectionData["vx1"].values
        x2 = DirectionData["vx2"].values
        y1 = DirectionData["vy1"].values
        y2 = DirectionData["vy2"].values
        z1 = DirectionData["vz1"].values
        z2 = DirectionData["vz2"].values

        temp1 = x1 * x2 + y1 * y2 + z1 * z2
        temp2 = np.sqrt(x1 * x1 + y1 * y1 + z1 * z1)
        temp3 = np.sqrt(x2 * x2 + y2 * y2 + z2 * z2)
        temp = np.arccos(temp1 / (temp2 * temp3))
        CreaseData['gamma13/pi'] = (temp + pi) / pi

        ax[0][1].set(xlabel=r'$\gamma_{13}$/$\pi$')
        ax[0][1].set(ylabel='CreaseEnergy')
        ax[0][1].set_xlim(1, 2)
        ax[0][1].plot(CreaseData['gamma13/pi'], CreaseData['CreaseALLSE'], linewidth=2.0, label=r'Energy-$\gamma_{13}$')
        ax[0][1].legend()
        ...
        ax[1][0].set(xlabel=r'$\phi_3$/$\pi$')
        ax[1][0].set(ylabel=r'$\phi_1$/$\pi$, $\phi_2$/$\pi$')
        phi1 = CreaseData["Crease1/pi"]
        phi2 = CreaseData["Crease2/pi"]
        phi3 = CreaseData["Crease3/pi"]
        ax[1][0].plot(phi3, phi1, linewidth=2.0, label=r'$\phi_1$/$\pi$')
        ax[1][0].plot(phi3, 2-phi2, linewidth=2.0, label=r'$2-\phi_2$/$\pi$')
        ax[1][0].set_ylim(1, 2)
        ax[1][0].set_xlim(1, 2)
        ax[1][0].legend()

        ax[1][1].set(xlabel=r'$\gamma_{13}$/$\pi$')
        ax[1][1].set(ylabel=r'$\phi_1$/$\pi$')
        ax[1][1].plot(CreaseData['gamma13/pi'], phi1, linewidth=2.0, label=r'$\phi_1$-$\gamma_{13}$')
        ax[1][1].set_ylim(1, 2)
        ax[1][1].set_xlim(1, 2)
        ax[1][1].legend()

        figname = "MiuraArray-Job" + str(jobnumber) + "-Angle-Energy-Relationship"
        fig.suptitle(figname)
        fig.savefig(figname + '.jpg')
        plt.show()
        fig_name_xlsx = "CreaseData" + str(jobnumber) + '.xlsx'
