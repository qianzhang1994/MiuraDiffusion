...
Path = 'C:\ABAQUSTemp\MiuraArray\Simulation0403'
# shutil.rmtree(Path)
# os.mkdir(Path)
...
def Crease(jobnumber, Stiffness, height, SectorAngle, InitialAngle, width, thickness, ArrayNumberX, ArrayNumberY,
           CreaseNumber, CutNumber):
    ...
    Model = CreaseArray(jobname, Stiffness, dx=dx, width=width, height=height,
                        ...
                        ArrayNumberY=ArrayNumberY, CreaseNumber=CreaseNumber, CutNumber=CutNumber)
    Model.Inpwrite()
...
jobnumber = 2615
for Sector in [85]:
    for CreaseNumber in range(10, 11):
